...
3. 拖拽：3D 里轨道左侧的白色圆点改距离，暂停时拖声源小球改起点；朝向用右侧面板的“朝向”快捷键和倾斜盘调整。
4. 预设：经典 8D / 歌手绕着你转 / 双环反向 / 上下翻滚 / 单点环绕（参考视频同款：所有音轨同一点、固定 12 秒一圈、混响 −10 dB）/ 三层环绕（人声顶层 75°、乐器高度层 35° 逆时针、鼓环绕层、贝斯固定）/ 斜向环绕（人声往右前翘 45°、乐器往左前翘 45° 逆时针）/ 立体交叉（人声左右竖环、乐器前后竖环、鼓水平环）。除单点环绕外都跟随歌曲小节。
...
- 把手：只有轨道相位 270°（左侧）的白色圆点 = 距离；暂停时拖小球改起点（仅圆 / 椭圆 / 螺旋 / 固定，相位可由位置反推）。倾斜不再放 3D 把手，统一由倾斜盘负责。
- 视觉层级：选中音轨的轨道粗而亮，其余细而淡（不透明度 0.16），未选中的小球缩小；不画头部到小球的连线；地面只留 1 m / 2 m 两圈。
...
### 10.3 面板与朝向
- 左栏：音轨名 + 静音 / 独奏；预设按钮由 `GET /api/presets` 生成。
- 右栏分【轨道】【混音】【空间】三页：轨道页 = 形状 → 朝向快捷键（水平 / 竖·左右 / 竖·前后 / 斜↗ / 斜↖）→ 倾斜盘 + 所在层 → 距离 → 形状专属参数 → 速度与转动方向 → “精细调节”（起点、高度、三个倾斜角）；混音页 = 音量、声像宽度、混响送出；空间页 = 房间、混响量、背后压暗、网格开关、重置视角。
