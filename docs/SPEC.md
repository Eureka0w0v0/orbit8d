...
5. 预设（只有一段时整个替换；多段时只改当前段的轨道与混响量）：经典 8D / 歌手绕着你转 / 双环反向 / 上下翻滚 / 单点环绕（参考视频同款：所有音轨同一点、固定 12 秒一圈、混响 −10 dB）/ 三层环绕（人声顶层 75°、乐器高度层 35° 逆时针、鼓环绕层、贝斯固定）/ 斜向环绕（人声往右前翘 45°、乐器往左前翘 45° 逆时针）/ 立体交叉（人声左右竖环、乐器前后竖环、鼓水平环）。除单点环绕外都跟随歌曲小节。
...
其余 6 个预设在 v2 的最大音色偏差都在 ±1.4 dB 内。
