...
3. 3D 场景：白色石膏人头 + 每条音轨一个彩色轨道环 + 发光声源小球，播放时小球沿轨道跑、声音同步移动。
4. 拖拽：3D 里轨道左侧的白色圆点改距离，暂停时拖声源小球改起点；朝向用右侧面板的“朝向”快捷键和倾斜盘调整。改的都是播放头所在的那一段。
...
web/（Vite + TypeScript + Three.js，无 UI 框架）
  ...
  src/scene/   白模人头、轨道环、把手、声源小球、拖拽交互、场景编辑纯函数（edit.ts）
...
- 轨道形状显示播放头所在段的轨道；小球位置按时间轴（含段落过渡、停顿、飞过头顶）。
- 把手：只有轨道相位 270°（左侧）的白色圆点 = 距离；暂停时拖小球改起点（仅圆 / 椭圆 / 螺旋 / 固定，相位可由位置反推：`start = 目标相位 − offset − 时间轴累积相位`）。倾斜不再放 3D 把手，统一由倾斜盘负责。
