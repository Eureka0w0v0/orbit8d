...
3. **时间轴**：播放条就是时间轴——彩色块是段落，上面叠着原曲的音量起伏；上方的黄条是“停顿”、粉条是“飞过头顶”。点一下跳到那里；拖段落之间的竖线调整分界（自动对齐小节）；拖事件条改位置、拖右端改长短（吸附到拍，按住 ⌥ 自由拖），点一下选中后按 Delete 删除。右侧面板和 3D 轨道显示的都是**播放头所在的那一段**。
4. **改轨道**（改的是播放头所在的那一段）：
