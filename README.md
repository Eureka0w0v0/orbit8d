...
3. **改轨道**：
   - 点击轨道或小球（或左栏的音轨名）选中音轨，右侧面板分【轨道】【混音】【空间】三页；
   - **朝向**：一键切换水平 / 竖·左右（左耳→头顶→右耳）/ 竖·前后（正前→头顶→脑后）/ 两个斜 45°；
     或者拖**倾斜盘**上的白点：往哪边拖轨道就往哪边翘，越靠边越竖直，双击回水平；
   - 3D 里拖轨道左侧的**白色圆点**改距离，暂停时可以直接拖小球改起点；数值细调在“精细调节”里。
4. **分层与预设**：右侧“所在层”一键切换环绕层（耳朵高度）/ 高度层 / 顶层（头顶绕圈）；左侧一键预设：经典 8D / 歌手绕着你转 / 双环反向 / 上下翻滚 / 单点环绕（整首歌一个声源，模仿常见的 8D 视频）/ 三层环绕 / 斜向环绕 / 立体交叉。
5. **混音**：左栏每条音轨有静音（M）、独奏（S）；“混音”页调音量、声像宽度、混响送出；“空间”页切换房间（小房间 / 大厅 / 教堂）、混响量和“背后压暗”（转到脑后时高频变暗的程度）。
